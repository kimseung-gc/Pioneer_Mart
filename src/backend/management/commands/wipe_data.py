...
class Command(BaseCommand):
    help = "Wipe all data except admin user and reset sequences"
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                AND c.relnamespace IN (
                    SELECT n.oid FROM pg_namespace n WHERE n.nspname NOT LIKE 'pg_%' AND n.nspname != 'information_schema'
                )
            """
            )
            ...
            for sequence in sequences:
                ...
                self.stdout.write(f"Reset sequence: {sequence}")
    ...
    def handle(self, *args, **kwargs):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "Wipe complete. Admin user preserved and sequences reset."
            )
        )
